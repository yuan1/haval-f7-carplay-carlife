...
swapmode() {
	...
	if [ -n "$mode" ]; then
		...
		carplay)
			#echo ""
			launcherusbreset "swaper-otg-carplay.lua" carplay
			;;
		carbit)
			#echo 'req:json:{"id":9,"cmd":"launch foryouapp","app":"fycarlife","dat":""}' >> /pps/services/app-launcher
			launcherusbreset "swaper-otg-carbit.lua" carbit
			;;
		carlife)
			echo 'req:json:{"id":9,"cmd":"launch foryouapp","app":"fycarlife","dat":""}' >>/pps/services/app-launcher
			launcherusbreset "swaper-otg-carlife.lua" carlife
			;;
		ipod)
	...
	elif [ "$mode" = "carplay" ]; then
		echo "bootmode::carplay" >>$config
	elif [ "$mode" = "carlife" ]; then
		echo "bootmode::carlife" >>$config
	else
